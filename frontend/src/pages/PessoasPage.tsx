...
export const PessoasPage: React.FC<PessoasPageProps> = ({ onPessoaClick }) => {
  ...
  const handleCreatePessoa = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await api.createPessoa({ name: newName, phone: newPhone });
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto rounded-xl border border-border-low-contrast bg-white shadow-sm">
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="bg-surface-subtle border-b border-border-low-contrast text-label-md font-bold text-on-surface-variant">
        ...
        >
          <form onSubmit={handleCreatePessoa} className="flex flex-col gap-5 p-6">
            ...
            <div className="flex justify-end gap-2.5 border-t border-border-low-contrast pt-5 mt-1">
              <Button type="button" variant="secondary" onClick={() => setIsCreateOpen(false)} disabled={actionLoading}>
                Cancelar
              </Button>
