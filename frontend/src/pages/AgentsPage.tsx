...
export const AgentsPage: React.FC<AgentsPageProps> = ({
  ...
  const totalPages = Math.ceil(groupedAgents.length / itemsPerPage);
  const paginatedGroups = groupedAgents.slice(
  ...
  return (
    <>
      ...
      <div className="flex flex-col rounded-2xl border border-border-low-contrast bg-white shadow-sm overflow-hidden">
        <div className="space-y-4 p-6 max-h-[calc(100vh-230px)] overflow-y-auto">
          {paginatedGroups.map((empGroup: { empresaId: number; empresaName: string; empresaLogo: string | null; agents: Agent[] }, idx: number) => {
        ...
        <Pagination
          ...
          totalItems={groupedAgents.length}
