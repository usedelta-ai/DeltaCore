...
async function main() {
  ...
  // Pepper + Bcrypt
  const peppered = crypto.createHmac('sha256', passKey!).update(plainPassword).digest('hex');
  const hashedPassword = bcrypt.hashSync(peppered, 10);
  ...
  if (check.rows.length > 0) {
    await pool.query(
      'UPDATE public.users SET password = $1, role = $2, active = true, updated_at = NOW() WHERE email = $3',
    ...
    await pool.query(
      ...
      ['Administrador DeltaAI', email, hashedPassword, 'superadmin']
    );
